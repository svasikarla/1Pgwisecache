  ...
  try {
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "You are a helpful assistant that analyzes URLs and extracts key information. For each URL, provide a category, headline, and summary. The category should be one of: Technology, Business, Science, Health, Entertainment, Sports, Politics, Education, Environment, or Other. The headline should be concise and engaging. The summary should be 3-4 key points, each on a new line starting with a bullet point (•). Each point should be a complete sentence."
        },
        {
          role: "user",
          content: `Please analyze this URL and provide a category, headline, and summary: ${url}`
        }
      ],
      temperature: 0.7,
      max_tokens: 500,
